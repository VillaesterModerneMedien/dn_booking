 ...
 */
namespace DnbookingNamespace\Component\Dnbooking\Administrator\View\Extras;
...
/**
 *  * View class for a list of extras.
 *
 * @since  1.0.0
 */
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_DNBOOKING_HEADLINE_EXTRAS'), 'list com_dnbooking');
        ...
        if (!$this->isEmptyState && $canDo->get('core.edit.state'))
		{
			...
			$childBar->publish('extras.publish')->listCheck(true);

			$childBar->unpublish('extras.unpublish')->listCheck(true);
            
            $childBar->archive('extras.archive')->listCheck(true);
            
            if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('extras.trash')->listCheck(true);
			}
        }
        
        if (!$this->isEmptyState && $this->state->get('filter.published') == -2 && $canDo->get('core.delete'))
		{
			$toolbar->delete('extras.delete')
